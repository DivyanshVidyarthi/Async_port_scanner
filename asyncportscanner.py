...
async def port_scanner(host,port,timeout=0.5): #Calling the port scanning function 
    try: #A method similar to if-else and instead of giving a conditon earlier it takes the code inside of it as a condition
        reader,writer = await asyncio.wait_for(asyncio.open_connection(host, port), timeout) #I know I know reader is not used anywhere but we use it to actually know if the port is getting the call or not, we can't remove it just like that
        writer.close()
...
async def main(host, port_start=1, port_end=8000, concurrency = 100): # function where scanning actually happens
    ...
    async def sem_scan(p): #A Semaphore: What it does is limiting the current resources in use by leaving the task if it takes to much time or power. 
        async with sem:
